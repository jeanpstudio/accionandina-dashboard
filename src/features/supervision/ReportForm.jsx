...
export default function ReportForm({ isViewMode = false }) {
  ...
  return (
    <div className="max-w-[1500px] mx-auto p-4 md:p-6 bg-gray-50/20 min-h-screen font-sans">
              ...
              <div className="flex flex-wrap gap-2">
                {Array.isArray(formData.social_links) &&
                  formData.social_links.map((l, i) => (
                    <div
                      key={i}
                      onClick={() => setPreviewSocialUrl(l)}
                      className="bg-brand/10 text-brand text-[9px] px-2.5 py-1.5 rounded-lg flex items-center gap-2 font-black border border-brand/5 max-w-full cursor-pointer hover:bg-brand/20 transition-all select-none hover:scale-[1.02]"
      ...
      {/* MODAL DE PREVISUALIZACIÓN DE REDES SOCIALES */}
      {previewSocialUrl && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-10 animate-in fade-in">
          <div 
            className="absolute inset-0 bg-gray-900/60 backdrop-blur-sm" 
            onClick={() => setPreviewSocialUrl(null)}
          />
          <div className="relative bg-white w-full max-w-lg rounded-[40px] shadow-2xl overflow-hidden flex flex-col animate-in zoom-in-95 duration-300">
            ...
            <div className="flex justify-between items-center p-6 border-b border-gray-100 bg-white">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-sm font-black text-gray-900 uppercase tracking-tighter">Ver Publicación</h3>
                  <p className="text-[10px] font-medium text-gray-400 truncate max-w-[200px] md:max-w-md">{previewSocialUrl}</p>
                </div>
              </div>
              <button 
                onClick={() => setPreviewSocialUrl(null)}
            ...
            {/* Content Body */}
            <div className="p-8 bg-gray-50 flex flex-col items-center justify-center text-center space-y-6">
              {/* Platform Detection & Visual Presentation */}
              {(() => {
                const url = previewSocialUrl.toLowerCase();
                if (url.includes("instagram.com") || url.includes("instagr.am")) {
                  return (
                    <div className="space-y-4 w-full flex flex-col items-center">
                      <div className="w-20 h-20 bg-gradient-to-tr from-[#f9ce34] via-[#ee2a7b] to-[#6228d7] rounded-3xl flex items-center justify-center text-white shadow-lg shadow-pink-500/25 animate-pulse">
                      ...
                      <span className="text-xs font-black text-pink-600 uppercase tracking-widest bg-pink-50 px-3 py-1 rounded-full border border-pink-100">Instagram Post</span>
                      <p className="text-xs text-gray-500 font-medium max-w-sm">
                        Instagram bloquea la previsualización directa en el dashboard por razones de seguridad. Usa los botones a continuación para abrir la publicación.
                      </p>
                    </div>
                  );
                  ...
                  return (
                    <div className="space-y-4 w-full flex flex-col items-center">
                      <div className="w-20 h-20 bg-[#1877F2] rounded-3xl flex items-center justify-center text-white shadow-lg shadow-blue-500/25 animate-pulse">
                        <svg className="w-10 h-10 fill-current" viewBox="0 0 24 24">
                          <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                        </svg>
                      </div>
                      <span className="text-xs font-black text-blue-600 uppercase tracking-widest bg-blue-50 px-3 py-1 rounded-full border border-blue-100">Facebook Post</span>
                      <p className="text-xs text-gray-500 font-medium max-w-sm">
                        Facebook bloquea la previsualización directa en el dashboard por razones de seguridad. Usa los botones a continuación para abrir la publicación.
                      </p>
                    </div>
                  );
                  ...
                  return (
                    <div className="space-y-4 w-full flex flex-col items-center">
                      <div className="w-20 h-20 bg-[#0A66C2] rounded-3xl flex items-center justify-center text-white shadow-lg shadow-sky-600/25 animate-pulse">
                        <svg className="w-10 h-10 fill-current" viewBox="0 0 24 24">
                          <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
                        </svg>
                      </div>
                      <span className="text-xs font-black text-indigo-600 uppercase tracking-widest bg-indigo-50 px-3 py-1 rounded-full border border-indigo-100">LinkedIn Post</span>
                      <p className="text-xs text-gray-500 font-medium max-w-sm">
                        LinkedIn bloquea la previsualización directa en el dashboard por razones de seguridad. Usa los botones a continuación para abrir la publicación.
                      </p>
                    </div>
                  );
                } else {
                  return (
                    <div className="space-y-4 w-full flex flex-col items-center">
                      <div className="w-20 h-20 bg-brand rounded-3xl flex items-center justify-center text-white shadow-lg shadow-brand/25 animate-pulse">
                        <Globe size={40} />
                      </div>
                      <span className="text-xs font-black text-brand uppercase tracking-widest bg-brand/5 px-3 py-1 rounded-full border border-brand/10">Publicación Externa</span>
                      <p className="text-xs text-gray-500 font-medium max-w-sm">
                        Esta publicación externa podría tener bloqueada la previsualización. Usa los botones a continuación para abrirla directamente.
                      </p>
                    </div>
                  );
                }
              ...
              {/* Botones de Acción */}
              <div className="w-full flex flex-col sm:flex-row gap-3 pt-4 border-t border-gray-200">
                <button
                  type="button"
                  onClick={() => {
                    window.open(
                      previewSocialUrl,
                      "_blank",
                      "width=650,height=800,scrollbars=yes,status=yes,resizable=yes"
                    );
                    setPreviewSocialUrl(null);
                  }}
                  className="flex-1 py-4 px-6 bg-brand text-white font-black text-xs uppercase tracking-wider rounded-2xl shadow-lg shadow-brand/15 hover:shadow-brand/20 hover:scale-[1.01] transition-all flex items-center justify-center gap-2"
                >
                ...
                <button
                  type="button"
                  onClick={() => {
                    window.open(previewSocialUrl, "_blank");
                    setPreviewSocialUrl(null);
                  }}
                  className="flex-1 py-4 px-6 bg-gray-900 text-white font-black text-xs uppercase tracking-wider rounded-2xl hover:bg-gray-800 transition-colors flex items-center justify-center gap-2"
                >
