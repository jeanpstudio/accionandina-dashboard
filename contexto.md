...
### [2026-06-24] Enlaces de Publicaciones Clickables y Modal de Vista Previa
*   **ReportForm.jsx**: Modificadas las etiquetas de enlaces de redes sociales (`social_links`) para que sean clickables. Al hacer clic, se abre un modal de vista previa inteligente con detección de plataforma (Instagram, Facebook, LinkedIn o enlace genérico).
*   **Modal de Vista Previa**: Muestra logotipos y colores temáticos de la red social correspondiente e incluye botones interactivos para abrir la publicación en una ventana emergente flotante (utilizando `window.open` con dimensiones controladas para saltar las restricciones de `<iframe>` impuestas por las plataformas) o en una nueva pestaña convencional del navegador. Se aplicó `e.stopPropagation()` en el botón "X" de eliminación de la etiqueta para evitar conflictos.
